...
<div class="card">
	...
	<div class="card-body">
		<form action="<?php echo Route::_('index.php?option=com_minitekwall&layout=edit&id=' . (int) $this->item->id); ?>" method="post" name="adminForm" id="widget-form" class="form-validate"><?php 
						...
						if ($corePlugin['type'] == $source['type'])
						{
							?><div class="col-12 col-md-3">
								<div class="source-tile p-4 text-center">
									...
									<button class="btn btn-success btn-source mt-3" data-source="<?php echo $corePlugin['type']; ?>" onclick="return false;">
			...
			// Extra plugins 
			if (isset($this->sources) && $this->sources && $otherPlugins) 
			{ 
				?><h4 class="header m-3 mt-5 p-3"><?php echo Text::_('COM_MINITEKWALL_WIDGET_OTHER_PLUGINS'); ?></h4>
				<div class="row"><?php 
						...
						if (!in_array($source['type'], $this->coreTypes))
						{
							?><div class="col-12 col-md-3">
								<div class="source-tile p-4 text-center">
									...
									<button class="btn btn-success btn-source mt-3" data-source="<?php echo $source['type']; ?>" onclick="return false;">
										<span class="fas fa-check" aria-hidden="true"></span>&nbsp;&nbsp;<?php echo Text::_('COM_MINITEKWALL_WIDGET_SELECT'); ?>
									</button>
								</div>
							</div><?php
