...
jQuery(document).ready(function($){
    ...
    // update on Joomla 4 trigger submitForm on advanced button
    var JASubmitButton = Joomla.submitbutton;
    Joomla.submitbutton = function(task, formSelector, validate){
        if(task == 'advanced'){
            return false;
        }
        JASubmitButton(task, formSelector, validate);
    }
});
...
// 					setTimeout(function(){
                        //   baseUrl+$clone.parents('joomla-field-media').find('img').attr('src')
