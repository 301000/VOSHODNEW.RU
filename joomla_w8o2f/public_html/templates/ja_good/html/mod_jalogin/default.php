...
<div class="ja-login-wrap">
	<ul class="ja-login<?php echo $params->get('moduleclass_sfx','')?>">
		<?php if($type == 'logout') : ?>
			...
			<?php else : ?>
				<li>
					<a class="login-switch" href="<?php echo JRoute::_('index.php?option=com_users&view=login');?>" onclick="showBox('ja-user-login','mod_login_username',this, window.event || event);return false;" title="<?php echo JText::_('TXT_LOGIN');?>">
						<span class="visible-xs visible-sm"><i class="fa fa-lock" aria-hidden="true"></i></span>
						<span class="hidden-xs hidden-sm"><?php echo JText::_('TXT_LOGIN');?></span>
					</a>
					...
					<div id="ja-user-login">
						...
						<form action="<?php echo JRoute::_('index.php', true, $params->get('usesecure')); ?>" method="post" name="form-login" id="login-form" >
							...
							<?php endif; ?>
							<input type="submit" name="Submit" class="button btn btn-primary" value="<?php echo JText::_('JABUTTON_LOGIN'); ?>" />
							<input type="hidden" name="option" value="com_users" />
						...
						if($option!='com_user' && $task != 'register' && $params->get('show_register_form', 1)) { ?>
							...
							<li>
								<a class="register-switch" href="<?php echo JRoute::_("index.php?option=com_users&task=registration");?>" onclick="showBox('ja-user-register','namemsg',this, window.event || event);return false;" >
									...
									<span class="hidden-xs hidden-sm"><?php echo JText::_('REGISTER');?></span>
								</a>
                              ...
                              JFormHelper::addFieldPath(JPATH_SITE . '/plugins/system/privacyconsent/src/Field');
                              JForm::addFormPath(JPATH_SITE . '/plugins/system/privacyconsent/forms');
						    						}else{
