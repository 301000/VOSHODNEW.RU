...
class Router extends \Joomla\CMS\Component\Router\RouterBase{
	...
	function build(&$query){
		...
		if (isset($query['Itemid']) && $query['Itemid']){
            ...
            if ($query['controller']=='category' && $query['task']!='' && isset($menuItem->query['category_id']) && !$menuItem->query['category_id']){
                $clearQuery = 0;
            }
            if ($controller=="category" && $query['task']=="view" && $query['category_id'] && !isset($catalias[$query['category_id']])) {
                if (!isset($menuItem->query['category_id'])) {
                    $clearQuery = 0;
                }
            }
            if ($query['controller']=='manufacturer' && $query['task']!='' && isset($menuItem->query['manufacturer_id']) && !$menuItem->query['manufacturer_id']){
                $clearQuery = 0;
            }
			if ($query['controller']=='manufacturer' && $query['task']!='' && isset($query['manufacturer_id']) && !isset($menuItem->query['manufacturer_id'])){
                $clearQuery = 0;
            }
            if ($query['controller']!=$menuItem->query['view']){
                $clearQuery = 0;
            }
            if (isset($menuItem->query['task']) && $query['task']!=$menuItem->query['task']){
                $clearQuery = 0;
            }
            if ($clearQuery){
