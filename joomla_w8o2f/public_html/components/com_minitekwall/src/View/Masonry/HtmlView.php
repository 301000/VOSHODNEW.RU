	...
	public function display($tpl = null)
 	{
		...
		{
			// Get pagination
			$this->pagination = $masonry_params['mas_pagination'];

			if ($this->pagination)
			{
				$masonry_pagination = $this->model->masonry_pagination;
				$masonry_pagination->getPaginationCss($masonry_params, $this->widgetID);
			}
			...
			$this->totalCount = $this->model->getItemsCount($this->widgetID);
			$this->remainingCount = $this->totalCount - $startLimit;
			$this->showRemaining = $masonry_params['mas_pag_remaining'];

			if ($this->totalCount < $startLimit)
			{
				$startLimit = $this->totalCount;
			}

			$lastPage = ceil(($this->totalCount - $startLimit) / $pageLimit);

			// Pages pagination
			$this->totalPages = ceil(($this->totalCount - $startLimit) / $pageLimit) + 1;

			// Get arrows
			$this->arrows = $masonry_params['mas_arrows'];
			...
			{
				$wa->useScript('com_minitekwall.infinite-scroll');
			}
